    ...
    (async () => {
      try {
        ...
        try {
          ...
          try {
            const { data: existing } = await supabase.from('companies').select('id').ilike('name', fallbackName).maybeSingle();
            if (existing?.id) cid = existing.id as number;
          } catch {}
          if (!cid) {
            const { data: created } = await supabase.from('companies').insert({ name: fallbackName }).select('id').single();
            cid = created?.id as number | undefined;
          }
          if (cid) {
